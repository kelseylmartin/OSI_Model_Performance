...
truth_final <- truth_final %>% 
  ...
  rownames_to_column(var = "REFERENCE") # Bring Deployment back
truth_final <- merge(truth_final, fwri_ref_key, by = "REFERENCE", all.x = T)


# Get a simple list of all species in the truth file
truth_species_list <- names(truth_final)[-c(1, 299:nrow(truth_final))] # Exclude 'Deployment' col and fwri_key_ref cols
...
for (s in 1:length(all_species_to_compare)){
  ...
  master_list <- inner_join(deploydat, subdat_manual, by = "Deployment", relationship = "many-to-many")
...
groundtruth_master <- groundtruth_master %>%
  ...
  dplyr::filter(!is.na(year)) # this is simply here in case the truth file is not up to date. If not, then there is no point in comparing to the manual counts so we will not need to proceed with that year. Mainly gets rid of artificial stations until we start including this for the east and west NOAA datasets.


# adding back in the info from the fwri_ref_key dataset
fwri_ref_key1 <- fwri_ref_key %>%
  dplyr::mutate(Deployment = ifelse(LAB %in% "FWRI"| (!is.na(as.numeric(YEAR)) & YEAR > 2023), SITE_ID, Deployment),
                REFERENCE = ifelse(LAB %in% "PC", Deployment, REFERENCE))

# Find rows in master that DO NOT have a matching Deployment in the key
unmatched_master <- anti_join(groundtruth_master, fwri_ref_key1, by = "Deployment")
# Check if the dataframe is empty
if (nrow(unmatched_master) > 0) {
  warning("Warning: Found Deployments in groundtruth_master missing from fwri_ref_key!")
  print(unique(unmatched_master$Deployment))
  # Optional: stop("Halting script due to missing reference keys.")
}
